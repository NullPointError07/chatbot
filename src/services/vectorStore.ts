    ...
    for (const file of files) {
      ...
      for (const item of items) {
        ...
        if (file.includes("bundle")) {
          ...
          type = "teacher";
          content = `Teacher: ${item.name}. Teaching Experience: ${
            item.currentOccupation?.teachingExperience || "not specified"
          } years. IELTS Score: ${item.education?.IELTSBandScore || "not specified"}. Type: ${
            item.teacherType || "teacher"
          }.`;
        }
